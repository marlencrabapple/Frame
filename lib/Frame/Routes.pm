...
method add ( $methods, $pattern, @args ) {
    ...
    # TODO: Plack::{App,Middleware,Component} stuff
    if ( $route_args{dest}{c} && !ref $route_args{dest}{c} ) {
        ...
        # This may not work because it isn't an instance
        # TODO: look at docs
...
method match ($req) {
  ...
  BRANCH: while ( my $curr = $$prev{branch} // $branches ) {
    ...
    continue {
        if ( $curr isa 'Frame::Routes::Route' && $i == scalar @path ) {
            if ( $curr->has_stops ) {
                foreach my $stop ( grep { $_->dest } $curr->stops->@* ) {
                    ...
                    my @placeholder_matches =
                      $self->prep_placeholders( $req, $stop, @dummies );

                    map { tie $_, 'Frame::Request::Placeholder::Dummy' }
                      @dummies;
        ...
        last BRANCH
          unless uniq( keys $$prev{branch}->%*, keys $$barren{$i}->%* );
...
method prep_placeholders ( $req, $route, @placeholders ) {
    for ( my $i = 0 ; $i < scalar @placeholders ; $i++ ) {
        $placeholders[$i] =
          { ( $route->placeholders )[$i] => $placeholders[$i] };
    }
