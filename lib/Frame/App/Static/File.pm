...
package Frame::App::Static::File;

class Frame::App::Static::File : isa(Plack::Component) :
  does(Frame::Controller);
...
field $root         : param;
field $file         : param;
field $content_type : param;
field $encoding     : param;
...
method call ($env){
    ...
    my ( $file, $path_info ) = $file || $self->locate_file($env);
...
sub locate_file ($env) {
    ...
    my @path    = split /[\\\/]/, $path,
      -1;    # -1 *MUST* be here to avoid security issues!
    if (@path) {
